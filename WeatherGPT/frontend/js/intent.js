/**
 * Client-Side Intent & Conversational Grounding Service
 * Powers the chatbot directly in the browser when running standalone or when backend is offline.
 * Zero hallucinations: all answers are directly calculated from Open-Meteo metrics.
 */
...
const IntentService = {
  processQuery(query, weatherData, cityName = 'this location', lang = 'en') {
    if (!weatherData || !weatherData.current) {
      return {
        answer: lang === 'hi' 
          ? 'क्षमा करें, मौसम डेटा अभी लोड नहीं हुआ है। कृपया पहले किसी शहर का चयन करें।'
          : 'Sorry, weather data is not currently loaded. Please select a city first.',
        recommendations: [],
        alerts: []
      };
    }
    ...
    // Rain & Umbrella Intent
    if (INTENT_PATTERNS.rain.test(q)) {
      if (isTomorrow) {
        const rainProb = tomorrow.rainProbability ?? 0;
        if (effectiveLang === 'hi') {
          answer = `कल ${cityName} में बारिश की संभावना ${rainProb}% है (मौसम: ${tomorrow.wmo?.description || 'सामान्य'})।`;
          if (rainProb >= 40) {
        ...
        if (effectiveLang === 'hi') {
          answer = `आज ${cityName} में बारिश की अधिकतम संभावना ${todayRainProb}% है। वर्तमान वर्षा दर ${currentRain} mm है और मौसम '${current.wmo?.description || 'सामान्य'}' है।`;
    ...
    // Temperature & Heat/Cold Intent
      ...
      if (isTomorrow) {
        ...
        if (effectiveLang === 'hi') {
          answer = `वर्तमान में ${cityName} में तापमान ${current.temperature}°C है (महसूस: ${current.feelsLike}°C)। आज तापमान ${today.minTemp}°C से ${today.maxTemp}°C के बीच रहेगा।`;
          if (today.maxTemp >= 38) {
          ...
          answer = `Currently in ${cityName}, it is ${current.temperature}°C (feels like ${current.feelsLike}°C). Today's range is ${today.minTemp}°C to ${today.maxTemp}°C.`;
          if (today.maxTemp >= 38) {
    ...
    // Humidity Intent
      ...
      if (effectiveLang === 'hi') {
        ...
        if (current.humidity >= 80) recommendations.push('हवा में नमी अधिक है, जिससे उमस महसूस हो सकती है।');
      } else {
        answer = `The relative humidity in ${cityName} is currently ${current.humidity}%.`;
        if (current.humidity >= 80) recommendations.push('High moisture levels may make it feel muggy and sticky.');
      }
    }
    ...
    // Wind Intent
      ...
      if (effectiveLang === 'hi') {
        ...
        if (current.windSpeed >= 30) recommendations.push('हवा की गति काफी तेज है, खुले में सावधानी बरतें।');
      } else {
        ...
        if (current.windSpeed >= 30) recommendations.push('Noticeably windy conditions; secure loose outdoor objects.');
      }
    }
    ...
    // Evening Intent
      ...
      if (effectiveLang === 'hi') {
        answer = `आज शाम ${cityName} में तापमान लगभग ${eveningHour.temperature}°C रहने का अनुमान है, बारिश की संभावना ${eveningHour.rainProbability}% तथा मौसम '${eveningHour.wmo?.description || 'शांत'}' रहेगा।`;
      } else {
    ...
    // Farmer & Agriculture Advisory Intent
